                ...
                if (files.Length <= 10)
                {
                    ...
                    foreach (var file in files)
                    {
                        if (!ValidateFile(file))
                        {
                            ...
                            inValidFiles += ", " + System.IO.Path.GetFileName(file.FileName);
                        }
                    }
                    if (allValid)
                    {
                    ...
                    {
                        ModelState.AddModelError("FileName", "The file must be gif, png, jpeg or jpg and less than 2MB in size.  The following files " + inValidFiles + " are not valid");
                    }
                }
                else
                {
                    ModelState.AddModelError("FileName", "Please upload only 10 files at a time");
                }
            }
            else
            {
